  ...
  return (
    <div>
      <header>
        ...
        justify-around lg:justify-center lg:items-start space-x-3 lg:space-x-6 pt-6 2xl:pt-10">
        ...
        space-y-4`}>
          ...
          <Link
            href='/#boxOffice'
            className="text-black xs:border-b xs:border-black xs:border-opacity-20 hover:font-bold">Box Office</Link>
          <Link
            href='/#popMovies'
            className="text-black xs:border-b xs:border-black xs:border-opacity-20 hover:font-bold
             whitespace-nowrap">Popular Movies</Link>
          <Link
            href='/#popTVSeries'
            className="text-black xs:border-b xs:border-black xs:border-opacity-20 hover:font-bold 
            whitespace-nowrap">Popular TV Shows</Link>
          <Link
            href='/#popActors'
            className="text-black xs:border-b xs:border-black xs:border-opacity-20 hover:font-bold 
            whitespace-nowrap">Popular Actors</Link>
          <Link
            href='/about'
            passHref
            className="text-black  xs:border-b xs:border-black xs:border-opacity-20 hover:font-bold">About</Link>
        </nav>
          <div>
            <button type="button" onClick={()=> mobileMenu()} className=" lg:hidden w-10 h-10 bg-black text-yellow-300 p-1">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 stroke-current text-yellow-300 lg:group-hover:text-black mt-1 lg:mt-4 
        ...
        <div className="flex flex-col place-items-center lg:ml-menu pt-4 z-45">
          ...
          rounded bg-white'>
          {searchResults.map((item, index) => (item.media_type ==="movie"? <Link
            key={index}
            href={`/title/${item.media_type}/${item.id}`}
            passHref
            className="py-2 hover:font-bold">{item.title}
            {item?.release_date && `\xa0(${item?.release_date?.substring(0, 4)})`}</Link>:
          item.media_type ==="tv"? <Link
            key={index}
            href={`/title/${item.media_type}/${item.id}`}
            passHref
            className="py-2 hover:font-bold">
            {item.name}{item?.first_air_date && `\xa0(${item?.first_air_date?.substring(0, 4)})`}
          </Link>: item.media_type ==="person" && item?.gender !==0 ?
          <Link
            key={index}
            href={`/name/${item.id}`}
            passHref
            className="py-2 hover:font-bold">{item.name}</Link>:null))}</div>}
        </div>
    </div>
  );
}
