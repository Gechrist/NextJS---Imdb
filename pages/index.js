  ...
  return (
    <div className="lg:ml-menu lg:w-main w-full flex flex-col pt-6 px-2 lg:px-10 overflow-y-auto">
      ...
      <section className="flex w-full flex-col space-y-4 xl:space-y-2">
        ...
        {!boxOffice.error? <EmblaCarousel haveAutoplay = {true} direction ='flex-col h-72 divide-y-2 divide-y-white' position =''> {boxOffice?.items.map((item,index) => ( 
              ...
              </div>))}</EmblaCarousel>:<p>{boxOffice.error}</p>}
      </section>
      <section className="flex w-full flex-col pt-24 mb-4 space-y-4 xl:space-y-2">
        ...
        {!popMovies.error? <EmblaCarousel haveAutoplay = {false} direction ='' position =''> {popMovies?.results.map((item,index) => (
          <div key={index} className='mx-1 py-4'>
            <Link href={`/title/movie/${item?.id}`} passHref>
              ...
              </div></Link></div>))}</EmblaCarousel>:<p>{popMovies.error}</p>}
      </section>
      <section className="flex w-full flex-col pt-24 mb-4 space-y-4 xl:space-y-2">
        <h1 className="text-2xl md:text-3xl 3xl:text-5xl" id='popTVSeries'>
            Popular TV Shows
        </h1>
        {!popTVSeries.error? <EmblaCarousel haveAutoplay = {false} direction ='' position =''> {popTVSeries?.results.map((item,index) => (
          <div key={index} className='mx-1 py-4'>
            <Link href={`/title/tv/${item?.id}`} passHref>
              ...
              </div></Link></div>))}</EmblaCarousel>:<p>{popTVSeries.error}</p>}
      </section>
      <section className="flex w-full flex-col py-24 mb-4 space-y-4 xl:space-y-2">
        ...
        {!popActors.error? <EmblaCarousel haveAutoplay = {false} direction ='' position =''> {popActors?.results.map((item,index) => (
          <div key={index} className='mx-1 py-4'>
            <Link href={`/name/${item?.id}`} passHref>
              ...
              </div></Link></div>))}</EmblaCarousel>:<p>{popActors.error}</p>}
      </section>
    </div>
  )
}
...
export async function getServerSideProps() {
  ...
  if (boxOffice.error)  boxOffice.error="There is an error with displaying the Box Office"
  let popMovies = await getData("movie/popular",'',API_OPTIONS);
  if (popMovies.error)  popMovies.error="There is an error with displaying Popular Movies"
  let popTVSeries = await getData("tv/popular",'',API_OPTIONS);
  if (popTVSeries.error) popTVSeries.error="There is an error with displaying Popular TV Shows"
  let popActors = await getData("person/popular",'',API_OPTIONS);
  if (popActors.error)  popActors.error="There is an error with displaying Popular Actors"
