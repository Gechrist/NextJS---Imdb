        ...
        <div className="lg:ml-menu lg:w-main w-full flex flex-col pt-6 px-2 lg:px-10">
          ...
          <div className="flex flex-row w-full h-80 xl:h-96">
            <section className="flex w-full flex-col space-y-4 xl:space-y-2">
                <h1 className="text-2xl md:text-3xl lg:text-5xl">{data?.title?data?.title:data?.name}</h1>
                ...
                <div className="flex flex-row pt-3">
                  ...
                  {data?.number_of_episodes} episodes</p>}
                </div>  
                <div className="flex flex-col">
                  <div className="flex flex-row flex-wrap">{mainCast?.map((item,index)=><Link key={index} 
          ...
          <nav className={`flex flex-row justify-between w-full mt-28 md:mt-20 lg:mt-26 h-auto divide-x divide-white border-2
           border-white`}>
            ...
            setIsShowCast(false)}} className="bg-transparent w-full active:bg-white active:text-black hover:font-bold p-2
             text-white">Play Trailer</button>
            <button type="button" onClick={() =>{setIsShowInfo(previsShowInfo => !previsShowInfo);setIsShowVideo(false);
            setIsShowCast(false)}} className="bg-transparent w-full  active:bg-white active:text-black hover:font-bold p-2
          ...
          isShowInfo?<Info info={data?.overview}/>:
          isShowCast?<Credits credits={data?.credits} type='title'/>:null}
          </div>
        </div>
     )
  }
