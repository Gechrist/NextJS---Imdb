...
const Header = () => {
  ...
  const mobileMenu = () => {
    isMobileMenu === 'hidden'
      ? setIsMobileMenu('xs:top-0 xs:right-0 xs:w-72 xs:h-mobile')
      : setIsMobileMenu('hidden');
  };
  const animationSearch = useTransition(isSearchInputOn, {
  ...
  useEffect(() => {
    ...
    const searchData = async () => {
      ...
      response?.success === false ||
        (response?.errors && setErrorMessage((errorMessage) => true));
    };
  ...
  return (
    <div>
      <header>
        <aside
          className="group lg:fixed flex flex-row relative bg-black lg:hover:bg-yellow-300 lg:w-menu lg:h-full 
        justify-around lg:justify-center space-x-3 lg:space-x-6 pt-6 2xl:pt-10"
        >
          ...
          <nav
            className={`${isMobileMenu} absolute top-32 lg:hidden lg:group-hover:flex flex lg:divide-none
        ...
        space-y-4`}
          >
            <button
              className="lg:hidden text-black mt-32 mb-6 text-center hover:bg-black hover:text-yellow-300 border-b-2 border-black"
              onClick={() => mobileMenu()}
            >
              CLOSE X
            </button>
          ...
          <div>
            <button
              type="button"
              onClick={() => mobileMenu()}
              className=" lg:hidden w-10 h-10 bg-black text-yellow-300 p-1"
            >
              <svg
                fill="currentColor"
                viewBox="0 0 20 20"
            ...
            <button
              ...
              onClick={() => {
                setIsMobileMenu('hidden');
                setIsSearchInputOn(
                  (previsSearchInputOn) => !previsSearchInputOn
                );
                setIsSearchResultsMenuOn(false);
                setSearchTerms('');
                setNotFoundMessage(false);
            ...
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-8 w-8 stroke-current text-yellow-300 lg:group-hover:text-black mt-1 lg:mt-4 
            pr-2 hover:scale-110"
                fill="none"
                viewBox="0 0 24 24"
                stroke="yellow"
              >
      ...
      <div className="flex flex-col place-items-center lg:ml-menu pt-4 z-45">
            ...
            item && (
              <animated.div style={styles}>
                <input
                  ...
                  value={searchTerms}
                  className="shadow appearance-none border border-yellow-300 rounded 
           py-2 px-3 text-gray-700 w-72 leading-tight focus:outline-none focus:shadow-outline z-45"
                  type="text"
                  placeholder="Search..."
                />
              </animated.div>
        ...
        <div
          className="flex absolute top-40 lg:top-20 w-12 sm:w-16 md:w-20 lg:w-auto h-auto rounded justify-center z-40 
          bg-transparent"
        >
          <LoadingIndicator />
        </div>
        <div className="mt-8 md:mt-2 text-center bg-grey-background">
          {notFoundMessage || errorMessage
            ? notFoundMessage && <h4>No Results Found</h4>
            : errorMessage && (
                <h4>Something went wrong. Please try again later.</h4>
              )}
        </div>
        {searchTerms && isSearchResultsMenuOn && searchResults?.length > 0 && (
          <div
            className="text-black absolute top-32 mt-1.5 lg:top-14 
            ...
            {searchResults.map((item, index) =>
              item.media_type === 'movie' ? (
                ...
                >
                  {item.title}
                  {item?.release_date &&
                    `\xa0(${item?.release_date?.substring(0, 4)})`}
                </Link>
              ) : item.media_type === 'tv' ? (
                <Link
                  key={index}
                ...
                >
                  {item.name}
                  {item?.first_air_date &&
                    `\xa0(${item?.first_air_date?.substring(0, 4)})`}
                </Link>
              ) : item.media_type === 'person' && item?.gender !== 0 ? (
                <Link
                  key={index}
                  href={`/name/${item.id}`}
                  passHref
